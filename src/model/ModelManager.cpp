...
void ModelManager::parseParamsTask(lv_obj_t* zkQr){
	...
	if(DEVICEPARAMSCONTROL->getParamId() > 0){
		...
		if(qrSize.length() > 0 && defQrSize != qrSizeInt){
			...
			layoutPositon_t position = ui_get_obj_layout_position(zkQr);
			position.mX = position.mX - (qrSizeInt - position.mWidth);
			position.mHeight = qrSizeInt;
			position.mWidth = qrSizeInt;
			ui_set_obj_layout_position(zkQr, position);
		}else if(defQrSize > 0){
			layoutPositon_t position = ui_get_obj_layout_position(zkQr);
			position.mX = position.mX - (defQrSize - position.mWidth);
		...
		if(isDisplayQr && defQrDisplay){
			ui_set_obj_visible(zkQr, isDisplayQr);
		}else{
		...
		if(qrLink.length() > 0 && qrLink != defCustomQr){
			...
			ui_update_QR_Content(zkQr, qrLink.c_str());
		}else if(defCustomQr.length() > 0){
			ui_update_QR_Content(zkQr, defCustomQr.c_str());
		}
	}
}
